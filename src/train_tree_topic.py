...
def train_tree_attn(input_batch:BatchedTree, target_batch,
               encoder:TreeLSTM, decoder:AttnDecoderRNN,
    ...
    input_lens = input_batch.batch_dgl_graph.batch_num_nodes()
    # if use_cuda:
    #     input_lens = input_lens.cuda()
    max_len = input_lens.max()
    batch_size = input_batch.batch_dgl_graph.batch_size
    seq_mask = (torch.arange(max_len).expand(len(input_lens.cpu()), max_len) >= input_lens.unsqueeze(1).cpu())
    if use_cuda:
