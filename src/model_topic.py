...
class EncoderRNN(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, topic_embedding_size, n_layers=2, dropout=0.5):
        ...
        self.gru = nn.GRU(embedding_size+topic_embedding_size, hidden_size, n_layers, dropout=dropout, bidirectional=True)

    def forward(self, input_seqs, input_lengths, topic_embedding, hidden=None):
        # Note: we run this all at once (over multiple batches of multiple sequences)
        embedded = self.embedding(input_seqs)  # S x B x E
        topic = topic_embedding.unsqueeze(0).expand(embedded.size(0), -1, -1)    # S x B x td
        stacked_embedded = torch.cat((embedded, topic), dim=2)  # S x B x (E+td)
        stacked_embedded = self.em_dropout(stacked_embedded)
        packed = torch.nn.utils.rnn.pack_padded_sequence(stacked_embedded, input_lengths, enforce_sorted=False)
